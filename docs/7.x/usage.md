...
Optionally, you could also install a PSR-11 dependency injection container, see [Dependency Injection](/7.x/dependency-injection) for more information.
...
- **Middleware Groups**: Define named middleware collections with `defineMiddlewareGroup()` and apply them by name with `middlewareGroup()` on routes, groups, or the router. See [Middleware](/7.x/middleware#middleware-groups).
...
See [Route Matching](/7.x/route-matching), [Middleware](/7.x/middleware), and [URL Generation](/7.x/routes#url-generation) for more details.
